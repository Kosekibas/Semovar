...
StateWork return_state;  // переменная состояния куда следует вернутся по выходу из блокировки
...
void loop()
{
  ...
  */
  ...
  else
  {
    ...
    //!  она тут не нужна, патамушта крутится выше автомата if (GetInput()) current_state = kBlocking; // если опрос входоввернул ошибку - идем в блокировку
  ...
  case kPwm:
    ...
    break;
  //------------------------------------------------запись лога
  //?кусок вырезан в функцию патамушта нех ему делать в автоматах
  /*case kDataLog:
    if (entry)
    { // первое вхождение
      previous_state = current_state;
  ...
  case kStop:
    ...
    //!  она тут не нужна, патамушта крутится выше автомата if (GetInput()) current_state = kBlocking; // если опрос входов вернул ошибку - идем в блокировку
    break;
    //-------------------------------------------------блокировка
  case kBlocking:
    if (entry)
    { // первое вхождение
      return_state=previous_state; //! состояние откуда пришли в которое возвратимся после блокировки. можно пиздецки запутаться
      previous_state = current_state;
      Serial.print("BLOCK ");
      ResetWork (); 
    ...
    switch (block_event) // по какой причинеб локировка
    {
