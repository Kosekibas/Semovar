...
#define TRIAC_TIMER 2 //таймер записи данных на карту
//----проверка времен 
long start_time=0;  
long time_work=0;
// состояние таймеров
...
#define time_interval_triac 1  //интервал для выдачи упраляющих импульсов на тиристор
...
int reg_max=100;	// макс. значение периода регулирования
int x=0;                      // Переменная координат алгоритма Брезенхема
int y=0;
int i = 1;
...
/*	modulator += setpoint;
    ...
    y=x;
    if (i >= 100)                   // После 100 раз, начинаем сначала
    {
      i = 0;
      y = 0;
    }
...
void loop()
{
  ...
  case kGetInput:
    ...
    pwm=analogRead(6); 
    out_servo=map(pwm, 0, 1023, 0, 255);
  ...
  case kStart:
    ...
    { // первое вхождение
      previous_state = current_state;
        //analogWrite(9, out_servo); //вывод на шим
    ...
    digitalWrite(RELAY, HIGH);    // включает реле
    current_state = kMode; //уходим в работу по режиму

    time_before_start=millis();
    break;
  //------------------------------------------------работа по режиму
  case kMode:
    if (entry)
  ...
  case kPwm:
    ...
    /*if (timers[TRIAC_TIMER] > time_interval_triac) //если прошло время на подготовку ответа
      {
    ...
    if (log_on)
    {
      current_state = kDataLog; //уходим в логгирование
    }
    else
    {
